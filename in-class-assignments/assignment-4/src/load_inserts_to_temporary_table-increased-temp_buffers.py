...
def getSQLcmnds(rowlist):
    temporary_table = get_temporary_table_name()
    cmdlist = []
    for row in rowlist:
        valstr = row2vals(row)
        cmd = f"INSERT INTO {temporary_table} VALUES ({valstr});"
        cmdlist.append(cmd)
    return cmdlist
...
def createTemporaryTable(conn):
    with conn.cursor() as cursor:
        ...
        print(f"Increasing temp_buffers to 1500 MB")
        cursor.execute("""SET temp_buffers = '1500MB';""")
        ...
        print(f"Created {temporary_table_name}")


def get_temporary_table_name():
    return TableName + "_temporary"


def load(conn, icmdlist):
    with conn.cursor() as cursor:
        start = time.perf_counter()
        print(f"Loading {len(icmdlist)} rows into temporary table ...")
        for cmd in icmdlist:
            # print (cmd)
            cursor.execute(cmd)
        ...
        cursor.execute(f"INSERT INTO {get_temporary_table_name()} SELECT * FROM {TableName};")
